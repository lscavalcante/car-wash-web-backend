...
import { UpdateService, UpdateServiceParams } from "../../../services/services/updateService";
import toLocaleDate from "../../../format/date/toLocaleDate";
...
export default function EditServicePage() {
  ...
  const [service, setService] = useState<ServiceDetailModel>();
  ...
  const initialValues: UpdateServiceParams = {
    id: Number(id),
    title: service?.title ?? '',
    client: service?.client.id ?? 0,
    finish: service?.finish ?? false,
    date_start: toLocaleDate(service?.date_start, "yyyy-mm-dd"),
    date_end: toLocaleDate(service?.date_end, "yyyy-mm-dd"),
    vehicle: service?.vehicle ?? 0,
    services: service?.services?.map(({ service_category, ...p }) => ({ id: p.id, title: p.title, price: p.price, service_category: service_category == 0 ? null : Number(service_category) })) ??
      [
        {
          id: null,
          title: '',
  ...
  const getService = async () => {
    try {
      ...
      setService(
        {
          ...service,
          services: service.services.map(({ ...p }) => ({ ...p, price: currencyFormatter(String(p.price)) })) as any
        }
      )
    } catch (error) {
  ...
  const updateService = async (value: UpdateServiceParams, formikHelpers: FormikHelpers<UpdateServiceParams>) => {
    try {
      await UpdateService(
        {
          ...value,
          services: value.services.map(({ service_category, ...p }) => ({ ...p, price: priceStringToDouble(String(p.price)), service_category: service_category == 0 ? null : Number(service_category) })) as any
        }
      );
      ...
      <div className="container mx-auto">
        <div className="flex flex-col items-center justify-center w-[450px] py-10 mx-auto">
          ...
          <Formik
            ...
            onSubmit={(values: UpdateServiceParams, formikHelpers: FormikHelpers<UpdateServiceParams>) => updateService(values, formikHelpers)}
          >
